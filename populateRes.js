console.log(
  "Este script escreve as coleções de Grupos e Bancas a database. Database passada como argumento ao node: node populateRes mongodb+srv://nomeUsuarioCluster:<senha>@cluster0-mbdj7.mongodb.net/databaseNome?retryWrites=true"
);
...
async.series([criarGrupos, criarBancas], function(erro, resultados) {
  if (erro) {
    console.log("ERRO FINAL: " + erro);
  } else {
    console.log(`${grupos.length}/25 grupos adicionados`);
    console.log(`${bancas.length} bancas adicionadas`);
  }
